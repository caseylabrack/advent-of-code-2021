...
for _, line in ipairs(points) do
  ...
  if line.x1 ~= line.x2 and line.y1 ~= line.y2 then -- diagonal line
    local signx = 1
    if line.x1 > line.x2 then signx = -1 end
    local signy = 1
    if line.y1 > line.y2 then signy = -1 end
    local ypos = line.y1
    for i=line.x1,line.x2,signx do
      map[ypos][i] = map[ypos][i] + 1
      ypos = ypos + signy -- in a diagonal line, x and y both change by |1|
    end
  end
